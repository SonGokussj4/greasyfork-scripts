...
const { Csfd, initIndexedDB } = requireLegacyCommonJsModule(path.resolve(__dirname, '../csfd-compare.js'));
...
describe('csfd.getParentNameFromUrl method', () => {
  ...
  test.each(cases)('Url: %p --> %p', async (url, expectedResult) => {
...
describe('csfd.getFilmNameFromFullUrl method', () => {
  const cases = [
    [
      'https://www.csfd.cz/film/1032817-naomi/1032819-don-t-believe-everything-you-think/recenze/',
      '/film/1032817-naomi/',
    ],
